...
// Citation: https://stackoverflow.com/a/49595208/5277750
func ReadImageFromFile(filePath string) *image.RGBA {
	f, err := os.Open(filePath)
	if err != nil {
		fmt.Println("Could not read image from file.")
		panic(err)
	}
	src, _, err := image.Decode(f)
	if err != nil {
		fmt.Println("Could not decode image read from file.")
		panic(err)
	}

	// Citation: https://blog.golang.org/image-draw#TOC_6.
	b := src.Bounds()
...
// Citation: My exercise 10 submission
func WriteImageToFile(filePath string, img *image.RGBA) {
...
// Citation: https://gobyexample.com/defer
func closeDeferable(f *os.File) {
	errClose := f.Close()
	if errClose != nil {
		panic(errClose)
	}
